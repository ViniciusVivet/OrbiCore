...
export function SyncIndicator() {
  ...
  return (
    <span className="ml-auto flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground" title={state.label}>
      <Icon className={`h-3.5 w-3.5 ${state.className}`} />
      <span className="max-w-36 truncate sm:max-w-none">{state.label}</span>
    </span>
  );
}
